...
class ActionThread(QThread):
    ...
    def run(self):
        if self.action == "uninstall":
            ...
            if verify.strip() in ("0", ""):
                self.result.emit(f"Uninstalled: {self.target}", True)
            else:
                self.result.emit(f"Uninstall may have failed for: {self.target}", False)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        title.setStyleSheet("font-size: 18px; font-weight: bold; color: #e0e8f0;")
        subtitle = QLabel("Scan for suspicious software, PUPs, adware, and proxy hijackers")
        subtitle.setStyleSheet("font-size: 12px; color: #707480;")
        ...
        self.status_label.setStyleSheet("color: #64c878; font-weight: bold; font-size: 14px;")
    ...
    def export_report(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save report", "security_report.txt", "Text files (*.txt)"
        )
        if path:
